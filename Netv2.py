...
# Reference: https://nn.labml.ai/graphs/gat/index.html
class GraphAttentionLayer(nn.Module):
    def __init__(self,
        ...
        # Dropout layer to be applied for attention scores
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor, attention_score_only: bool = False) -> torch.Tensor:
        """Forward pass of the GraphAttentionLayer module.

        Args:
            h (torch.Tensor): input node embeddings of shape [n_nodes, in_features]
            adj_mat (torch.Tensor): adjacency matrix of shape [n_nodes, n_nodes, n_heads]
            attention_score_only (bool): whether to return only the attention scores. Default to False.

        Returns:
            torch.Tensor: output node embeddings of shape [n_nodes, out_features]
        """
        n_nodes = h.shape[0]  # Number of nodes

        # Initial transformation
        g: torch.Tensor = self.linear(h)
        g = g.view(n_nodes, self.n_heads, self.n_hidden)

        # Calculate attention scores
        g_repeat = g.repeat(n_nodes, 1, 1)  # [g1, g2, ..., gn, g1, g2, ..., gn, ...]
        g_repeat_interleave = g.repeat_interleave(n_nodes, dim=0)  # [g1, ..., g1, g2, ..., g2, ..., gn, ..., gn]
        g_concat = torch.cat([g_repeat_interleave, g_repeat], dim=-1)  # [g1||g1, g1||g2, ..., g1||gn, g2||g1, g2||g2, ..., g2||gn, ...]
        g_concat = g_concat.view(n_nodes, n_nodes, self.n_heads, 2 * self.n_hidden)  # [gi||gj]

        e: torch.Tensor = self.activation(self.attn(g_concat))  # [n_nodes, n_nodes, n_heads, 1]
        ...
        # Mask out the attention scores for non-adjacent nodes
        e = e.masked_fill(adj_mat == 0, float('-inf'))

        # Normalize attention scores and apply dropout
        a: torch.Tensor = self.softmax(e)
        a = self.dropout(a)

        if attention_score_only:
            return a
...
class GCN_Module(nn.Module):
    def __init__(self, cfg):
        ...
        self.gat_list = torch.nn.ModuleList([ GraphAttentionLayer(NFG, NFG, 1) for i in range(NG) ])
    ...
    def forward(self,graph_boxes_features,boxes_in_flat):
        ...
        for i in range(NG):
            ...
            # Graph convolution
            adj_mat = torch.ones(B*N, B*N, self.gat_list[i].n_heads, device=relation_graph.device)
            attn_score = self.gat_list[i](graph_boxes_features.squeeze(0), adj_mat, attention_score_only=True)
            relation_graph = relation_graph * attn_score
