...
class UTDataset(data.Dataset): # create a dataset
    def __init__(self,bboxes,frames,labels, num_frames, image_size,feature_size,num_boxes=4,
                 data_augmentation=False):
        
        self.bboxes = bboxes
        self.frames = frames
        self.labels = labels
        self.image_path = "/data-ext/UT_dataset/completed/images/"
        ...
        for seq_name, frame_labels in self.labels.items():
        ...
        # print(len(self.list_frames), len(self.list_bboxes), len(self.list_labels)) # 17745
        # exit()
        # count = 0
    ...
    def __getitem__(self,index):
        ...
        sample = self.load_samples_sequence(self.list_frames[index], self.list_bboxes[index], self.list_labels[index]) # Load every frame 
    ...
    def load_samples_sequence(self,select_frame, select_bboxes, select_labels):
        ...
        list_select_labels.append(select_labels)
        select_frame = list_select_frame
        select_bboxes = list_select_bboxes
        select_labels = list_select_labels
...
def UT_get_bboxes(seqs):
    data = {}
    data_path = "/data-ext/UT_dataset/completed/bounding_boxes/"
    for sname in seqs:
...
def UT_get_frames(seqs): 
    data = {}
    data_path = "/data-ext/UT_dataset/completed/images/"
    for sname in seqs:
...
def UT_get_labels(seqs):
    data = {}
    data_path = "/data-ext/UT_dataset/completed/annotations/"
    for sname in seqs:
...
def build_ut(cfg):
    ...
    train_labels = UT_get_labels(trn_seq)  # a dict with keys as seq names and values as frame id dicts. In frame id dict, key is frame id and value is a dict with key as human pair ("0-1") and value as interaction label
