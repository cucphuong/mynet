...
from Netv1 import BasenetFgnnMeanfield
...
class_samples = [2708, 3046, 914, 876, 1236, 15584]
# Total number of samples
total_samples = sum(class_samples)
# Frequency of each class
class_frequency = [n / total_samples for n in class_samples]
# Alpha for each class (inverse of frequency)
alpha = [1 - f for f in class_frequency]
alpha_classes = torch.tensor(alpha)


class FocalLoss_multi(nn.Module):
    def __init__(self, alpha=alpha_classes, gamma=5., reduction='mean'):
        super().__init__()
        if alpha is not None:
            if isinstance(alpha, (list, tuple)):
                alpha = torch.tensor(alpha)
            assert alpha.dim() == 1, "alpha must be a 1D tensor"
        self.alpha = alpha
        self.gamma = gamma
...
class_samples = [21463, 5903]  # Number of samples for each class
total_samples = sum(class_samples)  # Total number of samples
class_frequency = [n / total_samples for n in class_samples]  # Frequency of each class
alpha = [1 - f for f in class_frequency]  # Alpha for each class (inverse of frequency)
alpha_binary = torch.tensor(alpha)  # Convert to tensor  


class FocalLoss_binary(nn.Module):
    def __init__(self, alpha=alpha_binary, gamma=5., reduction='mean'):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.reduction = reduction
    ...
    def forward(self, inputs, targets):
        CE_loss = F.cross_entropy(inputs, targets, reduction='none')
        pt = torch.exp(-CE_loss)
        alpha_t = self.alpha[targets.data.view(-1)].to(inputs.device)
...
def train_net(cfg):
    ...
    if cfg.model == "backbone":
        ...
        model.loadmodel(cfg.backbone_model_path)
    ...
    for epoch in range(start_epoch, start_epoch + cfg.max_epoch):
        ...
        test_func(validation_loader, model, device, epoch, cfg)
        
        # if epoch % cfg.test_interval_epoch == 0:# evaluation
        #     test_info = test_func(validation_loader, model, device, epoch, cfg)
        #     if (test_info['actions_acc']+test_info['interactions_acc'])/2 > best_acc:
        #         best_epoch = epoch
        #         best_acc = (test_info['actions_acc']+test_info['interactions_acc'])/2
        #     if cfg.model=="backbone":
        #         for m in model.modules():
        #             if isinstance(m, Basenet):
        #                 filepath=cfg.result_bb_path+'/epoch'+str(epoch)+'.pth'
...
def train_func(data_loader, model, device, optimizer, epoch, cfg):
    ...
    for batch_data in data_loader:
        ...
        # forward
        actions_scores, interaction_scores = model((batch_data[0], batch_data[1], batch_data[3]), batch_data_full[5], batch_data_full[6], batch_data_full[7])
        ...
        interactions_in = batch_data[4].reshape((batch_size, num_frames, int(cfg.num_boxes * (cfg.num_boxes - 1)/2)))
        ...
        interactions_in = interactions_in.reshape((batch_size * num_frames, int(cfg.num_boxes * (cfg.num_boxes - 1)/2),))
        ...
        for bt in range(batch_size * num_frames):
            ...
            interactions_in_nopad.append(interactions_in[bt, :int(N * (N - 1)/2)])
        ...
        # Create the Focal Loss function
        focal_loss_multi = FocalLoss_multi()
        focal_loss_binary = FocalLoss_binary()
        # Compute the losses
        actions_loss = focal_loss_multi(actions_scores, actions_in)
        interactions_loss = focal_loss_binary(interaction_scores, interactions_in)
...
def test_func(data_loader, model, device, epoch, cfg):
    ...
    with torch.no_grad():
        for batch_data in data_loader:
            ...
            # forward
            actions_scores, interactions_scores = model((batch_data[0], batch_data[1], batch_data[3]), batch_data_full[5], batch_data_full[6], batch_data_full[7])
            ...
            interactions_in = interactions_in.reshape(
                (batch_size * num_frames, int(cfg.num_boxes * (cfg.num_boxes - 1)/2),))
            bboxes_num = bboxes_num.reshape(batch_size * num_frames, )
            for bt in range(batch_size * num_frames):
                N = bboxes_num[bt] # get num_bboxes (people) of each frame
                actions_in_nopad.append(actions_in[bt, :N]) # eliminate padding of bboxes
                interactions_in_nopad.append(interactions_in[bt, :int(N * (N - 1)/2)]) # eliminate padding
    ...
    interactions_precision, interactions_recall, interactions_F1, support = precision_recall_fscore_support(
        interactions_labels_global, interactions_pred_global, beta=1, average=None)

    print('actions_precision:', actions_precision)
    print('actions_recall:', actions_recall)
    print('actions_F1:', actions_F1)
    print('interactions_precision:', interactions_precision)
    print('interactions_recall:', interactions_recall)
    print('interactions_F1:', interactions_F1)
    print("Finish")
